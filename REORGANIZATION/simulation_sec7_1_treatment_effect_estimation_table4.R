...
estCR = est1 = est2 = est3 = est4 = array(0, c(ITR, 5000, 2)) # 4 methods, 2 treatment effect models
...
while( itr <= ITR){
		...
		if( mean(apply(MAPD, 1, function(id) w[id[1],id[2]])) < 
			mean(apply(M1, 1, function(id) w[id[1],id[2]]))*.75 ) next;

		if( max(apply(MAPD, 1, function(id) w[id[1],id[2]])) ==
			max(apply(M1, 1, function(id) w[id[1],id[2]])) ) next;



		w_caliper = w
		if( max(apply(MAPD, 1, function(id) w[id[1],id[2]])) > 
			max(apply(M1, 1, function(id) w[id[1],id[2]])) ){
			cut = quantile(apply(MAPD, 1, function(id) w[id[1], id[2]]), .98)
		} else cut = 200

		w_caliper[w_caliper>cut] = 100*max(w)
		...
		if( max(apply(MAPD, 1, function(id) w[id[1],id[2]])) > 
			max(apply(M1, 1, function(id) w[id[1],id[2]])) ){
			cut = quantile(apply(MAPD, 1, function(id) w[id[1], id[2]]), .99)
		} else cut = 200
		...
		# MAPD_caliper1 = MAPD_caliper
		# outcome model Scenario 2
		 y0 = exp((x1+x2)/2)+rnorm(n,0,1)
		 y1 = y0+.5
		# # for scenario 2 
		y1heter = y0+(abs(x1))
		...
		for(i in 1:5000){
			...
			trt = sample(1:n)[1:(n/2)]
			trt1 = rep(0, n)
			trt1[trt] = 1
			# ATE estimation			
			Y1 = y0*(1-trt1) + y1*trt1
			estCR[itr,i,1] = mean(Y1[trt1==1]) - mean(Y1[trt1==0])

			Y1 = y0*(1-trt1) + y1heter*trt1
			estCR[itr,i,2] = mean(Y1[trt1==1]) - mean(Y1[trt1==0])
...
estCR_temp1 = estCR_temp2 = est1_temp1 = est1_temp2 = est2_temp1 = est2_temp2 =
est3_temp1 = est3_temp2 = est4_temp1 = est4_temp2 = matrix(0, 100, dim(est4)[2])

for(i in 1:dim(est4)[2]){
	estCR_temp1[,i] = estCR[,i,1] - ate[,1]
	estCR_temp2[,i] = estCR[,i,2] - ate[,2]
...
ITR1 = itr-1
round(c(mean(apply(estCR_temp1[1:ITR1,], 1, function(x) sqrt(mean(x^2)))),
mean(apply(est1_temp1[1:ITR1,], 1, function(x) sqrt(mean(x^2)))),
...
round(c(mean(apply(estCR_temp2[1:ITR1,], 1, function(x) sqrt(mean(x^2)))),
mean(apply(est1_temp2[1:ITR1,], 1, function(x) sqrt(mean(x^2)))),
